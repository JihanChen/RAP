...
public class Project implements java.io.Serializable {
    ...
    public Parameter findParameter(int parameterId, ParameterType parameterType) {
        for (Module module : getModuleList()) {
            for (Page page : module.getPageList()) {
                for (Action action : page.getActionList()) {
                    if (ParameterType.REQUEST.equals(parameterType)) {
                        for (Parameter parameter : action.getRequestParameterList()) {
                            if (parameter.getId() == parameterId) {
                                return parameter;
                            }
                        }
                    }else if (ParameterType.RESPONSE.equals(parameterType)){
                        for (Parameter parameter : action.getResponseParameterList()) {
                            if (parameter.getId() == parameterId) {
                                return parameter;
                            }
                        }
                    }else if (ParameterType.RETURNCODE.equals(parameterType)){
                        for (Parameter parameter : action.getReturnCodeList()) {
                            if (parameter.getId() == parameterId) {
                                return parameter;
                            }
                        }
    ...
    public void removeParameter(int id, Session session) {
        ...
        Parameter requestParameter = findParameter(id, ParameterType.REQUEST);
        if(requestParameter != null ){
            Iterator<Action> iterator = requestParameter.getActionRequestList().iterator();
            while (iterator.hasNext()) {
                iterator.next().getRequestParameterList().remove(requestParameter);
                session.delete(requestParameter);
            }
            return;
        }


        Parameter responseParameter = findParameter(id, ParameterType.RESPONSE);
        if(responseParameter != null ){
            Iterator<Action> iterator = responseParameter.getActionResponseList().iterator();
            while (iterator.hasNext()) {
                iterator.next().getResponseParameterList().remove(responseParameter);
                session.delete(responseParameter);
            }

            return;
        }

        Parameter returnCodeparameter = findParameter(id, ParameterType.RETURNCODE);
        if(returnCodeparameter != null ){
            Iterator<Action> iterator = returnCodeparameter.getActionReturnList().iterator();
            while (iterator.hasNext()) {
                iterator.next().getReturnCodeList().remove(returnCodeparameter);
                session.delete(returnCodeparameter);
            }
            return;
        }

        // 都没有就去查询子类结果参数信息
        Parameter childparameter = findChildParameter(id);
        if (childparameter != null && childparameter.getComplexParameterList() != null) {
            for (Parameter pComplex : childparameter.getComplexParameterList()) {
                pComplex.getParameterList().remove(childparameter);
                session.delete(childparameter);
            }
        }
