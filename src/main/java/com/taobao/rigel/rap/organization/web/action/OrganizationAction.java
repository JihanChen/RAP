...
import com.taobao.rigel.rap.project.vo.ProjectVO;
...
public class OrganizationAction extends ActionBase {
    ...
    public String index() {
        if (isUserLogined()) {
            corporationList= organizationMgr.getCorporationList();
            projects = this.projectMgr.getProjectList();
            for (Project p : projects) {
                ProjectVO dto=new ProjectVO();
                dto.setId( Integer.valueOf(p.getId()));
                dto.setName(p.getName());
                dto.setDesc(p.getIntroduction());
                dto.setStatus(p.getLastUpdateStr());
                dto.setAccounts( p.getMemberAccountListStr());
                dto.setCreator( p.getUser().getUserBaseInfo());
                dto.setRelated( Boolean.valueOf(p.getUser().getId() != getCurUserId()));
                dto.setGroupId(p.getGroupId());
                dto.setCorporationId(p.getCorporationId());
                projectList.add(dto);
            }
            return SUCCESS;
        } else {
