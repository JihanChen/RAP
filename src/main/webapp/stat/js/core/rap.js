...
(function() {
    ...
    p.findNextParameter = function(actionId, parameterId) {
        ...
        if (action) {
            var requestParameterList = action.requestParameterList,
                ...
                returnParameterList = action.returnCodeList,// return-code
                returnParameterListNum = returnCodeList.length;// return-code
        ...
        // return-code
        for (i = 0; i < returnParameterListNum; i++) {
            p = returnParameterList[i];
            if (p.id == parameterId) {
                if (i + 1 < returnParameterListNum) {
                    return returnParameterList[i + 1].id;
                }
                return -1;
            }
        }
    ...
    /**
     * add is return-code parameter  begin
     */
    p.isReturnCode = function(actionId, parameterId) {
        var action = this.getAction(actionId);
        if (action) {
            var list = action.returnCodeList,
                listNum = list.length;
    ...
    /**
     * add is return-code parameter end
     */
    ...
    p.addAction = function(obj, addExisted, isCopy) {
        ...
        if (!addExisted || addExisted === 'mount') {
            ...
            obj.returnCodeList =[];  // return-code
    ...
    /**
     * return-code 返回错误吗
     * add return parameter begin
     */
    p.addReturnParameter = function(actionId) {
        var returnCodeList;
        var pNew = newParameter();
        if(this.getAction(actionId).returnCodeList == undefined){
            returnCodeList = [];
            this.getAction(actionId).returnCodeList = returnCodeList;
        }
        this.getAction(actionId).returnCodeList.push(pNew);
        return pNew.id;
    };
    /**
     * return-code 返回错误吗
     * add return parameter end
     */
        ...
        for (var i = 0; i < n; i++) {
            ...
            for (var j = 0; j < pageListCount; j++) {
                ...
                for (var k = 0; k < actionListCount; k++) {
                    ...
                    /* return-code begin */
                    var returnCodeList = action.returnCodeList;
                    if(returnCodeList == undefined){
                        returnCodeList = [];
                    }
                    var returnCodeListCount = returnCodeList.length;
                    /* return-code end */
                    ...
                    // return-code
                    for (l = 0; l < returnCodeListCount; l++) {
                        parameter =  returnCodeList[l];
                        if (parameter.id == id) {
                            returnCodeList.splice(l, 1);
                            return 0;
                        }
                        if (removeParameterRecursively(parameter, id) === 0) {
                            return 0;
...
(function() {
    ...
    var ws       = rap.workspace,       // workspace module
        ...
        TEMPLATE = {            // static template
            ...
            "RETURN_RESPONSE_BEGIN"                : "<h2>返回码说明</h2> <table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-identifier\">返回码</td><td class=\"head-name\">说明</td><td class=\"head-remark\">备注</td></tr>",
            "RETURN_RESPONSE_BEGIN_EDIT"           : "<h2>返回码说明</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-op\">OP</td><td class=\"head-identifier\">返回码</td><td class=\"head-name\">说明</td><td class=\"head-remark\">备注</td></tr>",
            "RETURN_RESPONSE_END"                  : "</table>",
            "RETURN_RESPONSE_PARAMETER_ADD_BUTTON" : "<div class='btns-container'><a href=\"#\" class=\"btn btn-info btn-xs\" onclick=\"ws.addParam('return'); return false;\"><i class='glyphicon glyphicon-plus'></i>添加参数</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>",
    ...
    ws.addParam = function(type, parentParamId) {
        ...
        } else if (type == "return") { // return-code 返回码描述
            newParamId = p.addReturnParameter(_curActionId);
        }else {
            ...
            try {
                ...
                if (obj.isOk) {
                    ...
                    if (obj.projectData.moduleList.length === 0) {
                        obj.projectData.moduleList = [{"id":ws.generateId(),"name":"某模块（点击编辑后双击修改）","introduction":"","pageList":[{"moduleId":ws.generateId(),"name":"某页面","introduction":"","id":ws.generateId(),"isIdGenerated":true,"actionList":[{"pageId":ws.generateId(),"name":"某请求","requestType":"1","requestUrl":"","responseTemplate":"","description":"","id":ws.generateId(),"requestParameterList":[{"id":ws.generateId(),"identifier":"reqParam","name":"某请求参数","remark":"","validator":"","dataType":"number","parameterList":[]}],"returnCodeList":[{"id":ws.generateId(),"identifier":"returnParam","name":"返回列表","remark":"","validator":"","dataType":"number","parameterList":[]}],"responseParameterList":[{"id":ws.generateId(),"identifier":"resParam","name":"某响应参数","remark":"","validator":"","dataType":"number","parameterList":[]}]}]}]}];
                    }
                    if (_draftData) {
            ...
            if (e.ctrlKey && paramId) {
                ...
                if (isOk) {
                    ...
                    // otherwise, create new parameter, than switch to that
                        ...
                        // return-code
                    } else if (p.isReturnCode(_curActionId, prevParamId)) {
                        ws.addParam('return');
                    }
                } else {
            ...
            switch (_editContext.key) {
                ...
                case "param-remark":
                    ...
                    // otherwise, create new parameter, than switch to that
                        ...
                        ws.addParam('return');   //return-code
                    }
                    break;
            }
        }
   ...
   function renderModuleTreeList() {
    ...
    function workspaceSearch(key, ev) {
        ...
        function getAHtml(a) {
            ...
            /* retrurn-code  begin 额外性校验返回错误吗是否为undefined 防止之前数据不存在而出错不显示页面*/
            var returnCodeList= a.returnCodeList;
            if(returnCodeList == undefined){
                returnCodeList = [];
            }
            var returnCodeListNum = returnCodeList.length;
            /* retrurn-code  end */

            sortParams(requestParameterList);
            sortParams(responseParameterList);
            sortParams(returnCodeList); /* retrurn-code  end */
            ...
            /* return-code 返回吗说明   begin*/
            str += _isEditMode ? TEMPLATE.RETURN_RESPONSE_BEGIN_EDIT : TEMPLATE.RETURN_RESPONSE_BEGIN;
            for (i = 0; i < returnCodeListNum; i++) {
                p = returnCodeList[i];
                str += getPTRHtml(p,0,1);
            }
            str += TEMPLATE.RETURN_RESPONSE_END;
            //}
            if (_isEditMode) {
                str += TEMPLATE.RETURN_RESPONSE_PARAMETER_ADD_BUTTON;
            }
            /* return-code 返回吗说明   end */
         ...
         */
        function getPTRHtml(param, level,isReturnCode) {
            var str = "",
            ...
            level = level || 0;
            var isReturnCode = isReturnCode || 0; // return-code
            ...
            if(!isReturnCode){  // return-code 不需要type显示
